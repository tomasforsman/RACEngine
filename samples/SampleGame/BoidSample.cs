...
public static class BoidSample
{
    public static void Run(string[] args)
    {
        ...
        // ─── Species Data ────────────────────────────────────────
        var speciesIds    = new[] { "White", "Blue", "Red" };
        ...
        void UpdateBoidSettings(Vector2D<int> windowSize)
        {
            ...
            foreach (var selfId in speciesIds)
            foreach (var otherId in speciesIds)
            {
                interactionMap[(selfId, otherId)] =
                    selfId == otherId
                        ...
                        {
                            ("White", "Blue") or ("White", "Red")
                                => new SpeciesInteraction(1.5f, 0f, 0f),
                            ("Blue", "White")
                                => new SpeciesInteraction(0f, 0f, 1.2f),
        ...
        void SpawnAllSpecies()
        {
            ...
            foreach (var id in speciesIds)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    world.SetComponent(e, new PositionComponent(
                        X: (float)(random.NextDouble() * 2.0 - 1.0),
                        Y: (float)(random.NextDouble() * 2.0 - 1.0)
                    ));
                    world.SetComponent(e, new VelocityComponent(
                        VelocityX: 0f,
                        VelocityY: 0f
                    ));
                    world.SetComponent(e, new BoidSpeciesComponent(
                        SpeciesId: id,
                        Scale:     scale
                    ));
                }
        ...
        void SpawnObstacles()
        {
            ...
            world.SetComponent(e, new ObstacleComponent(Radius: 0.2f));
        }
        ...
        void DrawSpecies(string filterId)
        {
            ...
            foreach (var (_, pos, vel, spec) in world.Query<PositionComponent, VelocityComponent, BoidSpeciesComponent>())
            {
                if (spec.SpeciesId != filterId) continue;
