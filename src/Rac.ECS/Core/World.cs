...
public sealed class World
{
        ...
        where T2 : IComponent
    {
        ...
        var (smallerPool, largerPool) = pool1.Count <= pool2.Count
            ? (pool1, pool2)
            : (pool2, pool1);
        ...
        where T3 : IComponent
    {
        ...
        var smallestPool = new[] { pool1, pool2, pool3 }
            .OrderBy(p => p.Count)
            .First();
    ...
    /// </summary>
    private Dictionary<int, IComponent> GetPool<T>() where T : IComponent
    {
        var componentType = typeof(T);
        if (!_components.TryGetValue(componentType, out var pool))
        {
